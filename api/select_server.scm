			...
			(let-values (((input-port output-port) (tcp-connect host port)))
				...
				(let loop ()
					(let ((buffer (if (connection-session-id cn) (read-packet cn) (read-buffer input-port))))
						(case (get-packet-id buffer)
							((#x00) (let ((packet (game-server-packet/key-packet buffer)) (pc (connection-packet-channel cn)))
								(let ((crypter (make-crypter (bytes-append (ref packet 'key) (bytes #xa1 #x6c #x54 #x87)))))
